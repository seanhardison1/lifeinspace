...
yrt <- osm_paths %>% 
  filter(name %in% c("Yampa Core Trail",
                     "Yampa River Trail")) %>% 
  dplyr::select(name)
...
gc1 <- 
  baseball$osm_polygons %>% 
  filter(leisure == "golf_course")

gc2 <- 
  baseball$osm_multipolygons %>% 
  filter(leisure == "golf_course") %>% 
  bind_rows(gc1) %>% 
  dplyr::select(name)
...
trails <- osm_all_roads$osm_lines %>% 
  filter(highway == "path" & !note %in% "golf cart path") %>%
  filter(!osm_id %in% c("864318615", "864318614", "858720398", "858720397","623988890",
                        "235035912","235012606","235035916","235213686","235213625",
                        "831385869","238317831","827165687")) %>% 
  dplyr::select(name) %>% 
  bind_rows(yrt)
