...
pk <- parking$osm_polygons %>% 
  st_transform(ncrs) %>% 
  st_intersection(.,ohill_sf)
...
snet4 <- sfnetworks::as_sfnetwork(residential) %>%
  st_as_sf("edges") %>% 
  dplyr::select(highway) %>% 
  smoothr::densify(.,n = 10) %>% 
  st_transform(ncrs) %>% 
...
out <-
  ...
  # parking
  geom_sf(data = pk, color = "transparent", fill = "grey40",
          alpha = 0.3) +
  
  # roads
  geom_sf(data = snet4, size = 0.2, color = "grey50",
          fill = 'grey85') +
  # buildings
...
ggsave(out, filename = here::here("figs/test4.svg"),
       width = 12, 
       height = 18.55,
       units = "in")
...
ggsave(trail_svg, filename = here::here("figs/ohill_trails.svg"),
       width = 12, 
       height = 18.55,
       units = "in")
...
ggsave(elev_grad_legend, filename = here::here("figs/ohill_elev_legened.svg"),
       width = 12, 
       height = 18.55,
       units = "in")
